...
On the other hand, it appears that we are not having our current machine learning models go through such a complex process of trial and error for hypothesis generation. In many cases, when a question is presented, machine learning models predict answers through a single inference based on fixed parameters. This seems too simplistic compared to the human process of hypothesis generation, which often involves repeatedly generating and testing questions and hypotheses.
...
\section{Complex Compositions of Question Construction, Hypothesis Generation, and Verification}
...
人間はその答えが未知であることを自ら自覚し、そのような場合に取るべき手段を過去から援用することで、段階的に不確実性を削減しているように思われます。これは仮説の生成だけじゃなくて、問いの構築でも仮説の検証でも言えること。それが汎用的な手法を構築する上で重要なこと。
