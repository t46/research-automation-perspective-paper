...
\textcolor{red}{ここに論文からの情報取得の自動化の話入れる？可能ならここではない論文のどこかで IR の枠組みの話してその流れで述べたい}
...
問いの構築と仮説の生成は科学における人間の創造性が発揮される場所として認識されてきました。例えば。。。

仮説の生成は既存の知識から未知の問いの答えを予測することです。これはすでに見たデータからまだ見ぬデータに対して予測をするという意味で、機械学習の定式化に還元されるということができます。特に、機械学習における質問応答タスクそのままであるということもできます。また、大きな分布シフトがあるような、言い換えるならばより複雑な分布を想定するような状況での機械学習の予測問題だと考えられます。

% 機械学習モデルは予測です。検証を伴わない予測という意味で仮説生成と同じ。広義の意味ではこの定義に吸収される。フレームとしては質問応答だし、ood 予測。

実際に、機械学習モデルは科学の仮説生成に広く使われています。例えば、タンパク質の立体構造の予測、新薬の予測、新物質の予測、などは機械による仮説の生成の例です。これらはドメインごとに具体的にやっていることは全然異なりうるため具体的な方法は異なりますが、それらが仮説を生み出しているという点で共通しています。
。

仮説の空間がどのようなものであるか、仮説を生成することが具体的にどのように表現されるかは研究分野によってさまざまです。例えば仮説はテキストで表現されるかもしれませんし、数式で表現される数理モデルかもしれませんし、ある複数のパラメータの組み合わせかもしれません。例えば、仮説を数式で表現するものでは、科学的データから背後にある法則を記述するシンボリックな数式を発見する研究が行われてきました \cite{kramer2023automated}。

このような詳細の違いはありますが、many scientific discoveries can be seen as search problems \cite{coley2020autonomous}. From this perspective, hypothesis generation can be thought of as an act of efficiently exploring a space and selecting a certain point within it. Especially since the exploration space in science can be vast, there have been attempts to develop AI that can efficiently explore this space.

Wang et al. have categorized and organized how AI is being used for hypothesis generation in science from the perspective of using it for black box prediction, aiding in the exploration of hypothesis space, and finding solutions in a differentiable hypothesis space \cite{wang2023scientific}.


このような事例は膨大であり、網羅的に紹介することは私の能力を超えているので、ここではこのくらい簡単に触れる程度でとどめさせてください。これまで見てきたように、機械による仮説の生成の試みはすでにたくさんあり、それらは多くの成果を上げています。個別の研究分野における AI の仮説生成への応用はまだ多くの課題が残っているものと思われますが、個別の研究ドメインへの理解を欠く私の議論はここで終わらせます的な。

\subsubsection{What Would be a General Hypothesis Generator?}
...
しかし、そのような汎用的な仮説生成AIを作ることが、単により強力な汎用機械学習モデルを作成することとどれほど異なるのか私にはまだわかっていません。前述したように、仮説の生成は質問応答であり、機械学習の予測問題に還元されるように思われるからです。ただしここまで抽象化して議論するのも意味がないかもしれません。経験からの予測というならば私たち人間がしているほぼすべての活動が経験からの予測だと言えるからです。なので厳密には、私が仮説生成という行為がそのような予測活動のうち具体的にどのような特徴を有しているものなのかがわかっていないといった方が正確かもしれません。これは私の能力不足によるところです。是非多くのフィードバックをお待ちしています。
...
人間のように汎用的に人間のような仮説を生成する AI を作ることまでまだ多くの課題があるのはそうだが、それはより強力な汎用モデルを作成する課題と重複するのではないかというのが懸念。そうであれば、特にここで強調する必要もなさそう。
...
\textcolor{red}{後で書く。そもそもここで何を言いたいか？何を持って検証とするかは複数あると上で話したけどここでそれについて少しだけ詳しくみようみたいな話し方をする？じゃあそれを話して出てくる帰結は？一緒？}
